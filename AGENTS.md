...
asks (e.g. "balikin task ini ke todo", "reset task ke todo", "batalin yang lagi
...
Setiap perubahan user-visible (fitur, perbaikan bug, UI, deployment) **WAJIB**
dicatat sebelum selesai:

1. Tambahkan entry `CHANGELOG` terbaru di **paling atas** array
   `lib/i18n/index.ts` (sebelum versi sebelumnya). Versi pakai format `vX.Y.Z`,
   tanggal ISO `YYYY-MM-DD`, dan `features` diterjemahkan **SEMUA 5 bahasa**
   (id/en/zh/ko/ja) dengan jumlah bullet sama persis di tiap bahasa.
2. Bump `version` di `package.json` (semantic versioning: **minor** untuk fitur
   baru, **patch** untuk perbaikan bug). Tampilan versi di Settings → Informasi
   Aplikasi membaca langsung dari `package.json`, jadi tidak perlu diubah manual
   — pastikan nilai di sana selalu sinkron.
3. Perubahan infrastruktur/deploy yang tidak terlihat pengguna cukup satu bullet
   singkat (mis. "CI/CD: ...").
4. Jalankan test i18n: `npx tsx --test "lib/i18n/*.test.ts"` — **wajib hijau**
   (test memastikan kamus key lengkap di 5 bahasa dan changelog lengkap). Entry
   changelog tidak menambah key kamus, tapi pastikan tidak merusak yang ada.
